...
**Acceptance criteria:**
- [x] Product sections are represented by typed IDs: `desktop`, `tasks`, `characters`, `memory`, `executors`, `privacy`.
- [x] App-level callbacks exist for task running, draft generation, memory confirmation/rejection, observation session control, and sync toggle.
- [x] No behavior is removed from the existing thin slice.

**Verification:**
- [x] Tests pass: `npm test`
- [x] Build succeeds: `npm run build`
- [x] Manual check: starting the app still shows PersonaDesk and can run an autonomous task.
...
**Acceptance criteria:**
- [x] Top-level app shell renders a persistent product header and section navigation.
- [x] Panel chrome is centralized in a reusable component.
- [x] Existing status colors and labels remain consistent.

**Verification:**
- [x] Tests pass: `npm test`
- [x] Build succeeds: `npm run build`
- [x] Manual check: switching sections does not reset localStorage-backed app state.
...
- [x] The UI has a visible navigation structure.
- [x] Existing autonomous task and memory workflows still work.
...
**Acceptance criteria:**
- [x] Desktop stage shows emotional characters and latest task status.
- [x] Desktop stage does not show executor registry, sync controls, or memory management controls.
- [x] Emotional characters still display role boundary labels and relationship context.

**Verification:**
- [x] Tests pass: `npm test -- src/App.test.tsx`
- [x] Build succeeds: `npm run build`
- [x] Manual check: desktop section reads as a companion surface, not a settings page.
...
**Acceptance criteria:**
- [x] Task room contains the autonomous task form and task cards.
- [x] Running a task from the task room still creates a delivered or blocked run using the real domain runtime.
- [x] Task character cards are visible in this section, not mixed into global dashboard clutter.

**Verification:**
- [x] Tests pass: `npm test -- src/App.test.tsx src/domain/tasks.test.ts`
- [x] Build succeeds: `npm run build`
- [x] Manual check: create a normal task and a risky task; normal delivers, risky blocks with approval request.
...
**Acceptance criteria:**
- [x] Character Studio shows emotional characters, task characters, and pending drafts.
- [x] Generating a draft does not activate it until confirmed.
- [x] Image file handling disclosure is visible when no vision executor is configured.

**Verification:**
- [x] Tests pass: `npm test -- src/domain/characterDrafts.test.ts src/App.test.tsx`
- [x] Build succeeds: `npm run build`
- [x] Manual check: generate a character draft with text and an image file, then confirm it; the character appears in the correct list.
...
**Acceptance criteria:**
- [x] Memory Center lists pending memory candidates.
- [x] Confirming a candidate creates a long-term memory.
- [x] Rejecting a candidate discards it without writing memory.

**Verification:**
- [x] Tests pass: `npm test -- src/domain/memory.test.ts src/App.test.tsx`
- [x] Build succeeds: `npm run build`
- [x] Manual check: run a task, open Memory Center, confirm the generated memory candidate.
...
- [x] `npm test` passes.
- [x] `npm run build` passes.
- [x] Desktop, Tasks, Characters, and Memory sections each have a distinct job.
- [x] Existing data persists while switching sections.
...
**Acceptance criteria:**
- [x] Executor Settings lists deterministic, model API, local model, local agent, ASR, and TTS executors.
- [x] Unconfigured and missing providers remain visible with status reasons.
- [x] No unconfigured provider can be triggered as if it were available.

**Verification:**
- [x] Tests pass: `npm test -- src/domain/executors.test.ts src/App.test.tsx`
- [x] Build succeeds: `npm run build`
- [x] Manual check: Executor Settings shows OpenAI-compatible API as unconfigured and Codex CLI as missing unless detected.
...
**Acceptance criteria:**
- [x] Privacy section can start and stop an observation session.
- [x] Observation summaries are stored only for allowlisted apps.
- [x] Sync toggle remains local and clearly says it is optional summaries only.
- [x] Local-only data classes are visible.

**Verification:**
- [x] Tests pass: `npm test -- src/domain/observation.test.ts src/domain/storage.test.ts src/App.test.tsx`
- [x] Build succeeds: `npm run build`
- [x] Manual check: start observation with Safari allowlisted, add a summary, stop the session, and confirm the summary remains local text only.
...
- [x] Executor and privacy sections do not imply unavailable integrations are working.
...
**Acceptance criteria:**
- [x] `App.tsx` no longer contains large panel markup.
- [x] UI behavior stays covered by existing tests.
- [x] No duplicated state mutation logic exists across pages.

**Verification:**
- [x] Tests pass: `npm test`
- [x] Build succeeds: `npm run build`
- [x] Manual check: all sections remain reachable and functional.
...
**Acceptance criteria:**
- [x] README lists the new product sections.
- [x] README still states what is real now and what is unconfigured.
- [x] A short UX note documents why management features live in the control console rather than the desktop stage.

**Verification:**
- [x] Tests pass: `npm test`
- [x] Build succeeds: `npm run build`
- [x] Documentation review: README matches the UI sections.
...
- [x] Rust tests still pass: `cargo test --manifest-path src-tauri/Cargo.toml`.
- [x] Security audit passes: `npm audit --audit-level=moderate`.
- [x] App can be manually opened and each section can be visited.
- [x] No feature is represented as functional unless backed by real state or real configured provider behavior.
