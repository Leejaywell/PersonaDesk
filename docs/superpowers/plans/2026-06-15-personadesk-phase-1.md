...
## Task 9: Add Character Draft Generation from Text and Image Metadata

**Files:**
- Modify: `src/domain/types.ts`
- Create: `src/domain/characterDrafts.ts`
- Test: `src/domain/characterDrafts.test.ts`
...
- [x] **Step 1: Write failing character draft tests**

Create `src/domain/characterDrafts.test.ts`:
...
describe("character draft generation", () => {
  it("creates a draft from text and image metadata without activating it", () => {
    ...
    expect(state.characterDrafts[0].disclosures).toContain("Image handling used file metadata only; no vision model is configured.");
  });
...
- [x] **Step 2: Run the draft test and verify it fails**

Run:

```bash
npm test -- src/domain/characterDrafts.test.ts
```

Expected: FAIL because `characterDrafts.ts` does not exist.

- [x] **Step 3: Implement draft generation**

Add `CharacterDraft` to `src/domain/types.ts` and `characterDrafts: CharacterDraft[]` to `PersonaDeskState`.

Create `src/domain/characterDrafts.ts` with:

- `createCharacterDraft(state, input)`
- `confirmCharacterDraft(state, draftId)`
- `rejectCharacterDraft(state, draftId)`

The implementation must disclose that image handling uses metadata only unless a configured vision executor exists. Confirming a draft adds a real `Character` to `state.characters`; rejecting removes the draft.

- [x] **Step 4: Add draft UI**

Add a Character Drafts panel to `src/App.tsx` with:

- Text import textarea.
- Optional image file input.
- Generate draft button.
- Draft review card with disclosures.
...
- [x] **Step 6: Commit character drafts**

```bash
git add src/domain/types.ts src/domain/defaultState.ts src/domain/characterDrafts.ts src/domain/characterDrafts.test.ts src/App.tsx src/App.test.tsx src/styles.css README.md docs/superpowers/plans/2026-06-15-personadesk-phase-1.md
git commit -m "feat: add honest character draft generation"
```
...
- Character drafts can be generated from text and optional image file metadata, and remain inactive until confirmed.
