...
import { addTaskRunCompanionReactions } from "./conversation";
import type {
  ApprovalRequest,
  Artifact,
  Executor,
...
export function runAutonomyCycle(state: PersonaDeskState, taskId: string): PersonaDeskState {
  ...
  if (executor.type === "model-api" || executor.type === "local-model") {
    const runningRun: TaskRun = {
      ...
      status: "running",
      assignedCharacters,
      taskTree: taskTree.map((step) => ({ ...step, status: "planned" })),
      executorCalls: [],
      decisions: [
        `Scheduled task as ${taskScheduleSummary(task)}`,
        `AI task generation dispatched to ${executor.displayName}.`
      ],
      logs: [
        `Dispatched API request to ${executor.displayName}...`
      ],
      validationResults: [],
      artifacts: [],
      openIssues: [],
      approvalRequests: [],
      acceptance: null,
      finalSummary: `Task is running on ${executor.displayName}...`
    };

    return appendRun(state, task.id, runningRun, "running");
  }
...
export function runTaskRevision(state: PersonaDeskState, taskId: string, previousRunId: string): PersonaDeskState {
  ...
  if (executor.type === "model-api" || executor.type === "local-model") {
    const runningRun: TaskRun = {
      ...
      decisions: [
        `Scheduled revision as ${taskScheduleSummary(task)}`,
        `AI task revision dispatched to ${executor.displayName}.`,
        `Revision feedback preserved: ${revisionFeedback}`
      ],
      logs: [
        `Dispatched revision API request to ${executor.displayName}...`
      ],
      validationResults: [],
      artifacts: [],
      openIssues: [],
      approvalRequests: [],
      acceptance: null,
      finalSummary: `Task revision is running on ${executor.displayName}...`
    };
...
export function deliverTaskRun(
  state: PersonaDeskState,
  taskId: string,
  runId: string,
  executorId: string,
  ...
  const run = state.taskRuns.find((r) => r.id === runId);
  ...
  const updatedRun: TaskRun = {
    ...run,
    status: passed ? "delivered" : "blocked",
    taskTree: run.taskTree.map((step) => ({ ...step, status: passed ? "completed" : "blocked" })),
    executorCalls: [
      ...run.executorCalls,
      createExecutorCall(executor, "AI task generation using LLM provider.", "succeeded", executorOutput)
    ],
    decisions: [
      ...run.decisions,
      `Task plan generated by AI executor: ${executor.displayName}.`,
      "Validated output against user goal, desired output, and privacy constraint."
    ],
    logs: [
      ...run.logs,
      `Plan created by Orion using ${executor.displayName}.`,
      "Vale validated the generated artifact against acceptance checks."
    ],
    validationResults,
    artifacts: [artifact],
    openIssues: validationOpenIssues(validationResults),
    acceptance: passed
      ...
      : null,
    finalSummary: passed
      ? `Delivered a validated plan using ${executor.displayName}.`
      : `Artifact from ${executor.displayName} needs user review.`
  };

  const updatedState = {
    ...state,
    tasks: state.tasks.map((t) => (t.id === taskId ? { ...t, status: (passed ? "delivered" : "blocked") as TaskStatus } : t)),
    taskRuns: state.taskRuns.map((r) => (r.id === runId ? updatedRun : r)),
    memoryCandidates: [
      ...state.memoryCandidates,
      {
        id: createId("memory-candidate"),
        proposedLayer: "task" as const,
        proposedOwnerCharacterId: null,
        proposedText: `AI Planner completed task: ${task.title} using ${executor.displayName}`,
        sourceEvent: runId,
  ...
  return addTaskRunCompanionReactions(updatedState, runId);
}
...
export function failTaskRun(
  ...
  const updatedRun: TaskRun = {
    ...run,
    status: "failed",
    taskTree: run.taskTree.map((step) => ({ ...step, status: "blocked" })),
    executorCalls: [
      ...run.executorCalls,
      createExecutorCall(executor, "AI task generation using LLM provider.", "failed", `Error: ${errorMessage}`)
    ],
    decisions: [
      ...run.decisions,
      `Task failed using executor ${executor.displayName}.`
    ],
    logs: [
      ...run.logs,
      `AI task call failed: ${errorMessage}`
    ],
    finalSummary: `Task execution failed: ${errorMessage}`
  };
